...
CREATE PROC spInsertworkareas @workareaid INT, @skill NVARCHAR(30)
AS
BEGIN TRY
	INSERT INTO workareas(workareaid,skill)
...
CREATE PROC spInsertworkers @workerid INT, @workername NVARCHAR(30), @phone nvarchar (20), @payrate money
AS
BEGIN TRY
	INSERT INTO workers(workerid,workername,phone,payrate)
...
CREATE PROC spInsertworkerareas @workerid INT, @areaid INT
AS
BEGIN TRY 
...
CREATE PROC spInsertworks @workid INT, @customename NVARCHAR(30), @customeraddress NVARCHAR(40), @workareaid INT,
@workdescription NVARCHAR(40), @startdate DATE, @endtime DATETIME
AS
BEGIN TRY
	INSERT INTO works(workid,customename,customeraddress,workareaid, workdescription,startdate,endtime)
	VALUES(@workid, @customename, @customeraddress, @workareaid, @workdescription, @startdate, @endtime)
END TRY
BEGIN CATCH
     DECLARE @msg NVARCHAR(1000)
	 SELECT @msg = ERROR_MESSAGE()
...
CREATE PROC spInsertworks @workid INT, @customename NVARCHAR(30), @customeraddress NVARCHAR(40), @workareaid INT, @workdescription NVARCHAR(40), 
@startdate DATE, @endtime DATETIME
AS
BEGIN TRY
	INSERT INTO works(workid,customename,customeraddress,workareaid, workdescription,startdate,endtime)
	VALUES(@workid ,@customename,@customeraddress,@workareaid,@workdescription,@startdate,@endtime)
...
CREATE PROC spUpdateworkareas @workareaid INT, @skill NVARCHAR(30)
AS
BEGIN TRY
	update workareas
	set	workareaid=@workareaid, skill=@skill
...
CREATE PROC spUpdateworkers @workerid INT, @workername NVARCHAR(30), @phone nvarchar (20),  @payrate money
AS
BEGIN TRY
	update workers
	set	workerid=@workerid, workername=@workername, phone=@phone, payrate=@payrate
...
CREATE PROC spUpdateworks @workid INT, @customename NVARCHAR(30), @customeraddress NVARCHAR(40), @workareaid INT, @workdescription NVARCHAR(40), @startdate DATE, @endtime DATETIME
AS
BEGIN TRY
	update works
	set workid=@workid, customename=@customename, customeraddress=@customeraddress, workareaid=@workareaid, workdescription=@workdescription, startdate=@startdate, endtime=@endtime
	where workid=@workid
END TRY
BEGIN CATCH
	 ;
...
CREATE PROC spUpdateworkerareas @workerid INT, @areaid INT
AS
BEGIN TRY 
	update workerareas
	set	workerid=@workerid, areaid=@areaid
	where areaid=@areaid
END TRY
BEGIN CATCH
	;
...
CREATE PROC spDeleteworkareas @workareaid INT
AS
BEGIN TRY
      DELETE workareas
	  WHERE workareaid = @workareaid
...
CREATE PROC spDeleteworkerpayments @workerid INT
AS
BEGIN TRY
...
CREATE PROC spDeleteworkers @workerid INT
AS
BEGIN TRY
      DELETE workers
	  WHERE workerid = @workerid
...
CREATE PROC spInsertworkerpayments @workid INT, @workerid INT, @totalworkhour float, @totalpayment money
AS
BEGIN TRY 
	INSERT INTO workerpayments(workid,workerid,totalworkhour,totalpayment)
...
AS 
SELECT wp.workerid,workername,phone,payrate,totalpayment
from workerpayments as wp
join workers wk on wp.workerid=wk.workerid
...
AS 
SELECT w.workid,wa.workareaid,customename,customeraddress,startdate,endtime,skill
from works w
