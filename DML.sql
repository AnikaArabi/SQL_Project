...
EXEC spInsertworks 501,'MD.Jony', '12 , Gulshan-1',1,'Swage pipe replacement ','2020-01-01','2020-8-04'
GO 
EXEC spInsertworks 502,'MD.Jowel', '15 , Bonani-3',2,'Garage electric wiring ','2020-01-06','2020-5-04'
GO 
EXEC spInsertworks 503,'MD.shajib', '44,Dhanmondi-1',3,'House cleaning ','2020-02-05','2020-9-04'
GO 
EXEC spInsertworks 504,'MD.Faruk','33,Rajarbag-4',4,'Garden water line replace ','2020-03-01','2020-7-04'
GO 
EXEC spInsertworks 505,'MD.shahin','15, Arambag-4',5,'New Electic line','2020-04-01','2020-06-04'
GO 
EXEC spInsertworks 506,'MD.Jamir','15, Arambag-5', 5 ,'Electrical Repair','2020-04-01', NULL
GO 
select* from works
Go
...
FROM workareas woa
INNER JOIN workerareas wa ON woa.workareaid = wa.areaid 
INNER JOIN workerpayments wp ON wa.workerid = wp.workerid 
INNER JOIN workers wo ON wa.workerid = wo.workerid AND wp.workerid = wo.workerid 
INNER JOIN works w ON wa.areaid = w.workareaid AND wp.workid = w.workid
GO
...
FROM workers wo
INNER JOIN workerareas woa ON wo.workerid = woa.workerid 
INNER JOIN workareas wa ON woa.areaid = wa.workareaid 
LEFT OUTER JOIN workerpayments wp ON wo.workerid = wp.workerid 
LEFT OUTER JOIN works w ON wp.workid = w.workid AND wa.workareaid = w.workareaid
GO
...
FROM cte 
LEFT OUTER JOIN workerpayments wp ON cte.workerid = wp.workerid 
LEFT OUTER JOIN works w ON wp.workid = w.workid AND cte.workareaid = w.workareaid
GO
